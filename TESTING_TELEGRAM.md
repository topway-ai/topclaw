...
This guide covers testing the Telegram channel integration for TopClaw.
...
Solution: Check user allowlist
  ...
  4. Run: topclaw onboard --channels-only
```
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time topclaw channel doctor`   |
...
```bash
# Maximum logging
RUST_LOG=trace topclaw channel start
...
# Validate config
cat ~/.topclaw/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
