#!/bin/bash
# TopClaw Telegram Integration Test Suite
...
if [ -f "target/release/topclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/topclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/topclaw 2>/dev/null || stat -c%s target/release/topclaw)
...
CONFIG_PATH="$HOME/.topclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'topclaw onboard' first"
    fi
else
    warn "No config file found - run 'topclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/topclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
