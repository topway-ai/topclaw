...
mod tests {
    ...
    fn normalize_schema_handles_default_postgres_schema() {
        ...
        assert!(normalize_schema("PUBLIC").is_none());
        assert_eq!(normalize_schema("topclaw"), Some("topclaw".into()));
    }
    ...
    async fn new_does_not_panic_inside_tokio_runtime() {
        let outcome = std::panic::catch_unwind(|| {
            MariadbMemory::new(
                "mysql://topclaw:password@127.0.0.1:1/topclaw",
                "public",
                "memories",
                Some(1),
                false,
    ...
    async fn integration_roundtrip_when_test_db_is_configured() {
        let Some(db_url) = std::env::var("TOPCLAW_TEST_MARIADB_URL")
            .ok()
            .filter(|value| !value.trim().is_empty())
        else {
            eprintln!("Skipping MariaDB integration test: set TOPCLAW_TEST_MARIADB_URL to enable");
            return;
        };

        let schema = format!("topclaw_test_{}", Uuid::new_v4().simple());
