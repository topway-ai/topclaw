...
/// A single memory entry stored by a backend.
#[derive(Clone, Serialize, Deserialize)]
pub struct MemoryEntry {
    /// Backend-scoped unique identifier.
    pub id: String,
    /// Human-chosen or runtime-generated lookup key.
    pub key: String,
    /// Stored memory text.
    pub content: String,
    /// Logical category used for routing and filtering.
    pub category: MemoryCategory,
    /// RFC 3339 timestamp string generated by the backend/runtime.
    pub timestamp: String,
    /// Optional conversation/session scope.
    pub session_id: Option<String>,
    /// Optional search relevance score for recall results.
...
/// Core memory trait.
///
/// Backends are expected to provide durable storage semantics appropriate to
/// their implementation while preserving the behavioral contract of store,
/// recall, list, and forget operations.
#[async_trait]
pub trait Memory: Send + Sync {
    ...
    ///
    /// The exact ranking strategy is backend-specific. Some backends implement
    /// keyword search only, while others combine lexical and semantic scoring.
    async fn recall(
        &self,
        query: &str,
        limit: usize,
