...
TopClaw paused Matrix sync to avoid an infinite retry loop. \
Resolve by deregistering the stale Matrix device for this bot account, resetting the local Matrix crypto store, then restarting TopClaw.",
        );
    ...
    pub fn new_with_session_hint(
        ...
        Self::new_with_session_hint_and_topclaw_dir(
            homeserver,
            access_token,
            room_id,
            allowed_users,
    ...
    pub fn new_with_session_hint_and_topclaw_dir(
        homeserver: String,
        access_token: String,
        room_id: String,
        allowed_users: Vec<String>,
...
mod tests {
    ...
    #[test]
    fn matrix_store_dir_is_derived_from_topclaw_dir() {
        let ch = MatrixChannel::new_with_session_hint_and_topclaw_dir(
            "https://matrix.org".to_string(),
            "tok".to_string(),
            "!r:m".to_string(),
            vec![],
            None,
            None,
            Some(PathBuf::from("/tmp/topclaw")),
        );

        assert_eq!(
            ch.matrix_store_dir(),
            Some(PathBuf::from("/tmp/topclaw/state/matrix"))
        );
    }

    #[test]
    fn matrix_store_dir_absent_without_topclaw_dir() {
        let ch = MatrixChannel::new_with_session_hint(
            "https://matrix.org".to_string(),
            "tok".to_string(),
            "!r:m".to_string(),
    ...
    fn otk_conflict_recovery_message_includes_store_path_when_available() {
        ...
        assert!(message.contains("/tmp/topclaw/state/matrix"));
    }
