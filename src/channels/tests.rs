...
impl Tool for DirectDesktopComputerUseTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let output = if action == "screen_capture" {
            "Screenshot saved: /tmp/direct-desktop-test.png (100x100). Sidecar data: {}".to_string()
        } else {
            format!("{action} ok. {{}}")
        };
        Ok(ToolResult {
...
async fn process_channel_message_confirm_auto_resumes_original_request() {
    ...
    assert!(sent.iter().any(
        |entry| entry.contains("BTC is currently around $65,000 based on latest tool output.")
    ));
}
...
async fn process_channel_message_falls_back_to_local_model_answer_on_provider_error() {
    ...
    assert!(sent_messages[0]
        .ends_with("I'm currently using provider `openrouter` with model `minimax/minimax-m2.7`."));
}
...
fn capability_tool_state_accounts_for_presence_exclusion_and_approval() {
    ...
    let approval_manager = ApprovalManager::from_config(&crate::config::AutonomyConfig::default());
    assert_eq!(
...
fn infer_capability_recovery_plan_flags_desktop_requests_for_computer_use_approval() {
    ...
    let msg = traits::ChannelMessage {
        ...
        sender: "topclaw_user".to_string(),
        reply_target: "chat-1".to_string(),
        content: "Open Google Chrome to https://example.com and scroll to the bottom.".to_string(),
        channel: "telegram".to_string(),
        timestamp: 0,
        thread_ts: None,
    };
...
async fn process_channel_message_web_request_with_gated_tool_creates_approval_request() {
    ...
    assert_eq!(sent.len(), 1);
    assert!(
        sent[0].contains("I can finish this, but I need supervised access to `web_fetch` first.")
    );
    ...
    assert_eq!(
        runtime_ctx
            .approval_manager
            .list_non_cli_pending_requests(Some("alice"), Some("telegram"), Some("chat-telegram"))
            .len(),
        1
    );
}
