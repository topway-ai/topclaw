...
async fn observability_config_default() {
    ...
    assert_eq!(o.runtime_trace_path, "");
    assert_eq!(o.runtime_trace_max_entries, 200);
}

#[test]
async fn observability_config_empty_trace_path_resolves_to_xdg_cache() {
    use crate::observability::runtime_trace::resolve_trace_path;
    let o = ObservabilityConfig::default();
    let workspace = tempfile::tempdir().unwrap();
    let path = resolve_trace_path(&o, workspace.path());
    assert!(
        !path.starts_with(workspace.path()),
        "empty runtime_trace_path should resolve to XDG cache, not workspace: {:?}",
        path
    );
    assert!(
        path.to_string_lossy().contains("cache"),
        "default trace path should be under cache dir: {:?}",
        path
    );
}
...
async fn schema_secrets_encrypt_config_secrets_encrypts_root_and_nested_fields() {
    ...
    assert!(config
        ...
        .telegram
        .as_ref()
        .is_some_and(|telegram| crate::security::SecretStore::is_encrypted(&telegram.bot_token)));
    assert!(config
        .agents
        .get("worker")
...
async fn config_save_atomic_cleanup() {
    let dir = std::env::temp_dir().join(format!("topclaw_test_config_{}", uuid::Uuid::new_v4()));
    fs::create_dir_all(&dir).await.unwrap();
...
async fn env_override_workspace() {
    ...
    assert_eq!(
        ...
        PathBuf::from("/custom/workspace/workspace")
    );
...
async fn persist_active_workspace_marker_is_written_to_selected_config_root_only() {
    ...
    // Marker is written to the default config dir so load_or_init can find it.
    assert!(
        default_marker_path.exists(),
        "marker should exist at default config dir"
    );

    let state: ActiveWorkspaceState =
        toml::from_str(&fs::read_to_string(&default_marker_path).await.unwrap()).unwrap();
    assert_eq!(PathBuf::from(state.config_dir), custom_config_dir);
...
async fn persist_active_workspace_marker_tolerates_restricted_default_home_root() {
    ...
    // When the default config dir is not writable, the marker falls back to
...
async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
    ...
    persist_active_workspace_config_dir(&custom_config_dir)
        .await
        .unwrap();
    // Marker exists in the default config dir.
    assert!(marker_path.exists());

    persist_active_workspace_config_dir(&default_config_dir)
        .await
        .unwrap();
    // Persisting the default config dir clears the marker.
    assert!(!marker_path.exists());
