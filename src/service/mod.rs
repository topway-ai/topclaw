...
const SERVICE_LABEL: &str = "com.topclaw.daemon";
const WINDOWS_TASK_NAME: &str = "TopClaw Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "topclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["topclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "topclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["topclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "topclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["topclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["topclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/topclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/topclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "topclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove topclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/topclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: topclaw service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "topclaw.service"]));
...
/// Check if the topclaw user exists and has expected properties.
...
fn check_topclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "topclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser topclaw && delgroup topclaw",
            "addgroup -S topclaw && adduser -S -s /sbin/nologin -H -D -G topclaw topclaw",
        )
    } else {
        ("userdel topclaw", "useradd -r -s /sbin/nologin topclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'topclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'topclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/topclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: topclaw user has home directory '{}' (expected /var/lib/topclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_topclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "topclaw"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_topclaw_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "topclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "topclaw"])
                .output()
                .context("Failed to create topclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create topclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: topclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create topclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create topclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "topclaw"])
            .output()
            .context("Failed to create topclaw user")?;
    ...
    println!("✅ Created system user: topclaw");
    Ok(())
}

/// Change ownership of a path to topclaw:topclaw
#[cfg(unix)]
fn chown_to_topclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["topclaw:topclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to topclaw:topclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_topclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_topclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "topclaw:topclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to topclaw:topclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_topclaw(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'topclaw' cannot write {} ({details}). \
             Re-run `sudo topclaw service install` and ensure ownership is topclaw:topclaw.",
            path.display(),
        );
    }
...
command_user="topclaw:topclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/topclaw/access.log"
error_log="/var/log/topclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/topclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo topclaw service install"
        );
    }
    ...
    let config_dir = Path::new("/etc/topclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/topclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_topclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by topclaw:topclaw)",
    ...
    chown_recursive_to_topclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/topclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "topclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/topclaw");
    println!("   Config path: /etc/topclaw/config.toml");
    println!("   Start with: sudo topclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("topclaw-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/topclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "TopClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/topclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/topclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"topclaw\""));
        assert!(script.contains("description=\"TopClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/topclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/topclaw daemon\""));
        assert!(!script.contains("env TOPCLAW_CONFIG_DIR"));
        assert!(!script.contains("env TOPCLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"topclaw:topclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/topclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/topclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/topclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/topclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/topclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/topclaw"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/topclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/topclaw/workspace'".to_string(),
                "topclaw".to_string()
            ]
        );
    }
