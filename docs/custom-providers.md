...
TopClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
- Auto mode: when your `custom:` endpoint resolves to `api.openai.com`, TopClaw will try WebSocket mode first (`wss://.../responses`) and automatically fall back to HTTP if the websocket handshake or stream fails.
- Manual override:
  - `TOPCLAW_RESPONSES_WEBSOCKET=1` forces websocket-first mode for any `custom:` endpoint.
  - `TOPCLAW_RESPONSES_WEBSOCKET=0` disables websocket mode and uses HTTP only.
...
TopClaw includes a first-class provider for [Tencent Hunyuan](https://hunyuan.tencent.com/):
...
TopClaw includes a first-class local provider for `llama-server`:
...
```bash
topclaw models refresh --provider llamacpp
topclaw agent -m "hello"
```

You do not need to export `TOPCLAW_API_KEY=dummy` for this flow.

## SGLang Server

TopClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
topclaw models refresh --provider sglang
topclaw agent -m "hello"
```
...
TopClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
topclaw models refresh --provider vllm
topclaw agent -m "hello"
```
