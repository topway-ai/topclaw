# Hardware Peripherals Design — TopClaw

TopClaw enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.
...
**Mental model:** TopClaw = brain that understands hardware. Peripherals = arms and legs it controls.
...
TopClaw runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.
...
TopClaw runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.
...
2. TopClaw identifies connected hardware (VID/PID, architecture)
...
2. TopClaw writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. TopClaw auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| TopClaw runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs TopClaw; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

TopClaw on Pi; GPIO via rppal or sysfs. No separate firmware.
...
1. **Startup:** TopClaw loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between TopClaw and peripherals:
...
- **topclaw-firmware** or **topclaw-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; TopClaw connects and discovers capabilities.
...
- [x] `topclaw hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `topclaw hardware introspect <path>`: memory map, peripheral list
...
- [x] TopClaw on Raspberry Pi (native GPIO via rppal)
...
- [x] `topclaw-esp32` firmware crate (`firmware/topclaw-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] TopClaw *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/topclaw-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full TopClaw *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
