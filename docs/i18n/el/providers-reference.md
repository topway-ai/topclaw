# Αναφορά Παρόχων TopClaw (Providers Reference)
...
Το runtime του TopClaw αναζητά διαπιστευτήρια με την εξής σειρά προτεραιότητας:
...
3. **Γενικές μεταβλητές**: Εφεδρικές μεταβλητές όπως οι `TOPCLAW_API_KEY` ή `API_KEY`.
...
- Τα μοντέλα συλλογιστικής (thinking models) υποστηρίζονται εγγενώς· το TopClaw φιλτράρει αυτόματα τα εσωτερικά metadata της συλλογιστικής.
...
Κλήση μέσω CLI: `topclaw agent --model hint:fast --message "..."`.
