...
- Το CLI `topclaw sop` διαχειρίζεται αυτή τη στιγμή μόνο ορισμούς: `list`, `validate`, `show`.
...
2. Δημιουργήστε έναν κατάλογο SOP, για παράδειγμα:
   ...
   ```text
   ~/.topclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.topclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Επαλήθευση και επιθεώρηση ορισμών:
   ...
   ```bash
   topclaw sop list
   topclaw sop validate
   topclaw sop show deploy-prod
   ```
