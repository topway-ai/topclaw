...
Αυτός ο οδηγός περιγράφει τη διαδικασία διαμόρφωσης των καναλιών επικοινωνίας (Telegram, Discord, κ.λπ.) στο TopClaw.
...
Οι ρυθμίσεις αποθηκεύονται στο αρχείο `~/.topclaw/config.toml`, στην ενότητα `[channels_config]`.
...
Σε πλατφόρμες όπως το Telegram και το Discord, μπορείτε να ελέγχετε το TopClaw με εντολές:
...
Το TopClaw υποστηρίζει την ανάγνωση εικόνων μέσω της σύνταξης:
`[IMAGE:<source>]`
...
5. **Επανεκκίνηση**: Κάθε αλλαγή στο `config.toml` απαιτεί επανεκκίνηση του `topclaw daemon`.

### Διαγνωστικά Μηνύματα (Logs)

Εκτελέστε το TopClaw με αυξημένο επίπεδο καταγραφής:
```bash
