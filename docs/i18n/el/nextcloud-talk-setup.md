...
Αυτός ο οδηγός περιγράφει τη διαδικασία ενσωμάτωσης του Nextcloud Talk με το TopClaw.
...
Στο αρχείο `~/.topclaw/config.toml`, προσθέστε την ενότητα `[channels_config.nextcloud_talk]`:
...
> **Συμβουλή**: Μπορείτε να χρησιμοποιήσετε τη μεταβλητή περιβάλλοντος `TOPCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` για να παρακάμψετε τη ρύθμιση του αρχείου.

## 3. Ρύθμιση Τελικού Σημείου (Endpoint)

Ξεκινήστε τον daemon του TopClaw για να εκθέσετε το webhook:
...
Εάν έχει οριστεί `webhook_secret`, το TopClaw επαληθεύει τις κεφαλίδες:
...
- Το TopClaw αγνοεί συμβάντα που προέρχονται από άλλα bots (`actorType = bots`).
...
3. Επιβεβαιώστε τη λήψη και την απάντηση από το TopClaw.
