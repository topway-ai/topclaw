# Προσθήκη Πλακετών και Εργαλείων — Οδηγός Υλικού TopClaw

Αυτός ο οδηγός εξηγεί πώς να προσθέσετε νέες πλακέτες υλικού και προσαρμοσμένα εργαλεία στο TopClaw.

## Γρήγορη Εκκίνηση: Προσθήκη Πλακέτας μέσω CLI

```bash
# Προσθήκη πλακέτας (ενημερώνει το ~/.topclaw/config.toml)

topclaw peripheral add nucleo-f401re /dev/ttyACM0
topclaw peripheral add arduino-uno /dev/cu.usbmodem12345
topclaw peripheral add rpi-gpio native   # για Raspberry Pi GPIO (Linux)
...
Με τη δυνατότητα rag-pdf, το TopClaw μπορεί να ευρετηριάσει αρχεία PDF:
...
2. Προσθήκη στις ρυθμίσεις — topclaw peripheral add my-board /dev/ttyUSB0
...
| Εντολή | Περιγραφή |
|---------|-------------|
| topclaw peripheral list | Λίστα ρυθμισμένων πλακετών |
| topclaw peripheral add <board> <path> | Προσθήκη πλακέτας (εγγραφή στο config) |
| topclaw peripheral flash | Μεταφόρτωση (flash) υλικολογισμικού Arduino |
| topclaw peripheral flash-nucleo | Μεταφόρτωση (flash) υλικολογισμικού Nucleo |
| topclaw hardware discover | Λίστα συσκευών USB |
| topclaw hardware info | Πληροφορίες chip μέσω probe-rs |
