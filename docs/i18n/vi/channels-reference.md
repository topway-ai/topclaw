...
Tài liệu này là nguồn tham khảo chính thức về cấu hình channel trong TopClaw.
...
6. **Trạng thái runtime cũ**: config đã thay đổi nhưng `topclaw daemon` chưa được khởi động lại.

---

## 1. Namespace cấu hình

Tất cả cài đặt channel nằm trong `channels_config` trong `~/.topclaw/config.toml`.
...
Khi chạy `topclaw channel start` (hoặc chế độ daemon), Telegram và Discord hỗ trợ chuyển đổi runtime theo phạm vi người gửi:
...
- Xem trước bộ nhớ cache model từ `topclaw models refresh --provider <ID>`.
- Đây là lệnh chat runtime, không phải lệnh con CLI.

## Giao thức marker hình ảnh đầu vào

TopClaw hỗ trợ đầu vào multimodal qua các marker nội tuyến trong tin nhắn:
...
Nếu `[channels_config.matrix]`, `[channels_config.lark]`, hoặc `[channels_config.feishu]` có mặt nhưng binary được build mà không có feature tương ứng, các lệnh `topclaw channel list`, `topclaw channel doctor`, và `topclaw channel start` sẽ ghi log rằng channel đó bị bỏ qua có chủ ý trong bản build này.
...
access_token = "syt_..."
user_id = "@topclaw:matrix.example.com"   # tùy chọn, khuyến nghị cho E2EE
...
TopClaw hỗ trợ hai backend WhatsApp:
...
5. Khởi động lại `topclaw daemon` sau khi thay đổi config.
...
```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Webhook|Channel" /tmp/topclaw.log
```
