...
Hướng dẫn này giải thích cách sử dụng gói Python `topclaw-tools` để gọi tool nhất quán với bất kỳ LLM provider nào tương thích OpenAI.

## Bối cảnh

Một số LLM provider, đặc biệt là các model Trung Quốc như GLM-5 (Zhipu AI), có hành vi gọi tool không nhất quán khi dùng phương thức text-based tool invocation. Core Rust của TopClaw sử dụng structured tool calling theo định dạng OpenAI API, nhưng một số model phản hồi tốt hơn với cách tiếp cận khác.
...
| Tool | Mô tả |
|------|-------|
| `web_search` | Tìm kiếm web (yêu cầu `TOPCLAW_BRAVE_API_KEY`) |
...
```python
from topclaw_tools import tool, create_agent
...
export TOPCLAW_BRAVE_API_KEY="your-brave-key"  # Optional, for web search

# Single message
topclaw-tools "What is the current date?"

# Interactive mode
topclaw-tools -i
```

## So sánh với Rust TopClaw

| Khía cạnh | Rust TopClaw | topclaw-tools |
|--------|---------------|-----------------|
