# Tài liệu tham khảo Providers — TopClaw
...
3. Biến môi trường dự phòng chung: `TOPCLAW_API_KEY`, sau đó là `API_KEY`
...
- Sau khi chuẩn hóa multimodal, TopClaw gửi payload hình ảnh qua trường `messages[].images` gốc của Ollama.
- Nếu chọn provider không hỗ trợ vision, TopClaw trả về lỗi rõ ràng thay vì âm thầm bỏ qua hình ảnh.
...
Vì TopClaw không thể tự động phát hiện, bạn có thể ghi đè trong `config.toml`:
...
Biến môi trường: `TOPCLAW_MODEL_SUPPORT_VISION=true`
...
- Khi đặt, ghi đè `TOPCLAW_CODEX_REASONING_EFFORT`.
- Không đặt sẽ dùng `TOPCLAW_CODEX_REASONING_EFFORT` nếu có, nếu không mặc định `xhigh`.
...
- Khám phá model: `topclaw models refresh --provider nvidia`
