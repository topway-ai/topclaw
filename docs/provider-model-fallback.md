...
This guide explains how to configure TopClaw to handle quota exhaustion and model failures gracefully using provider and model fallback chains.
...
2. If Gemini returns 429 (rate limited), TopClaw tries the fallback
...
When fallback occurs, TopClaw logs:
