# TopClaw Security Improvement Roadmap
...
TopClaw already has **excellent application-layer security**:
...
## Comparison: TopClaw vs PicoClaw vs Production Grade

| Feature | PicoClaw | TopClaw Now | TopClaw + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`topclaw audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/topclaw/audit.log"
sign_events = true
max_size_mb = 100
...
topclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
topclaw audit --user @alice --since 24h
topclaw audit --risk high --violations-only
topclaw audit --verify-signatures

# Sandbox test
topclaw sandbox --test
# → Testing isolation...
...
**TopClaw is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, TopClaw becomes:
...
**Value**: Transforms TopClaw from "safe for testing" to "safe for production"
