...
1. Resolve latest workflow run IDs:
   - `gh run list --repo topclaw-labs/topclaw --workflow feature-matrix.yml --limit 1`
   - `gh run list --repo topclaw-labs/topclaw --workflow ci-run.yml --limit 1`
2. Enumerate check/job names and compare to this mapping:
   - `gh run view <run_id> --repo topclaw-labs/topclaw --json jobs --jq '.jobs[].name'`
