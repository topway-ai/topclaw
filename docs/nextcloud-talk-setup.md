...
This guide covers native Nextcloud Talk integration for TopClaw.
...
Add this section in `~/.topclaw/config.toml`:
...
topclaw daemon
# or
topclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, TopClaw verifies:
...
- TopClaw accepts both payload variants:
  - legacy Talk webhook payloads (`type = "message"`)
  - Activity Streams 2.0 payloads (`type = "Create"` + `object.type = "Note"`)
- TopClaw ignores bot-originated webhook events (`actorType = bots` or `actor.type = "Application"`).
- TopClaw ignores non-message/system events.
...
3. Confirm TopClaw receives and replies in the same room.
