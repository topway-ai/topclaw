...
it pulls `ghcr.io/topclaw-labs/topclaw:latest` and tags it locally before running.
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/topclaw-labs/topclaw:latest`)
- `--skip-install`
- `--provider <id>`
