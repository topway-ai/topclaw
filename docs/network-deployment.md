# Network Deployment — TopClaw on Raspberry Pi and Local Network

This document covers deploying TopClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
- Telegram channel works: TopClaw polls Telegram API (outbound)
...
2. Start a tunnel:
   ...
   ```
   Or use `topclaw tunnel` (see tunnel docs).

3. TopClaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- TopClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `topclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `topclaw daemon` service).
...
- [ ] Run `topclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
TopClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `topclaw` system user (created during install)
...
This creates:
- Init script: `/etc/init.d/topclaw`
- Config directory: `/etc/topclaw/`
- Log directory: `/var/log/topclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo topclaw service install` automatically prepares `/etc/topclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `topclaw` service user.

If no prior runtime state is available to migrate, create `/etc/topclaw/config.toml` before starting the service.
...
| Command | Description |
|---------|-------------|
| `sudo rc-service topclaw start` | Start the daemon |
| `sudo rc-service topclaw stop` | Stop the daemon |
| `sudo rc-service topclaw status` | Check service status |
| `sudo rc-service topclaw restart` | Restart the daemon |
| `sudo topclaw service status` | TopClaw status wrapper (uses `/etc/topclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/topclaw/access.log` |
| Errors/stderr | `/var/log/topclaw/error.log` |
...
sudo rc-service topclaw stop
sudo rc-update del topclaw default

# Remove init script
sudo topclaw service uninstall
```
...
- The service runs as the `topclaw:topclaw` user (least privilege)
- Config must be at `/etc/topclaw/config.toml` (explicit path in init script)
- If the `topclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo topclaw service install`
- [ ] Enable: `sudo rc-update add topclaw default`
- [ ] Start: `sudo rc-service topclaw start`
- [ ] Verify: `sudo rc-service topclaw status`
- [ ] Check logs: `/var/log/topclaw/error.log`
