# TopClaw Development Environment

A fully containerized development sandbox for TopClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`topclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `topclaw` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/topclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.topclaw` (mounted to `/topclaw-data/.topclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.topclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
```bash
cp dev/config.wasm.staging.toml target/.topclaw/config.toml
```
...
- Rust compilation, tests, and audit/deny tools run in `topclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=topclaw-target`) to speed repeat local image builds.
