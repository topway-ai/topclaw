...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.topclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.topclaw/auth-profiles.json
    let Some(home) = topclaw::test_capabilities::home_dir_from_env() else {
        eprintln!("⚠️  Skipping test: neither HOME nor USERPROFILE is set");
        return Ok(());
    };

    if let Err(reason) = topclaw::test_capabilities::check_writable_dir(&home) {
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: topclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: topclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.topclaw/auth-profiles.json (with expired token)
    let provider = topclaw::providers::create_provider("gemini", None)?;
