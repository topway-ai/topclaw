...
#
# Environment:
#   TOPCLAW_CONFIG_DIR — override config dir (default: /home/spex/.topclaw)
#   TOPCLAW_BIN        — override binary path (default: ./target/release/topclaw)
...
export TOPCLAW_CONFIG_DIR="${TOPCLAW_CONFIG_DIR:-/home/spex/.topclaw}"
TOPCLAW_BIN="${TOPCLAW_BIN:-${REPO_ROOT}/target/release/topclaw}"
TIMEOUT="${TIMEOUT:-120}"
...
run_agent_test() {
  ...
  output=$(set +o pipefail; yes 2>/dev/null | timeout "${TIMEOUT}" ${TOPCLAW_BIN} agent -m "$message" $agent_flags 2>&1) || rc=$?
...
log "Config dir:  ${TOPCLAW_CONFIG_DIR}"
log "Binary:      ${TOPCLAW_BIN}"
log "Timeout:     ${TIMEOUT}s"
log "Log file:    ${LOG_FILE}"

# Fix active_workspace.toml if it points to a temp dir
AWF="${TOPCLAW_CONFIG_DIR}/active_workspace.toml"
if [ -f "$AWF" ]; then
  current_dir=$(grep -oP 'config_dir\s*=\s*"\K[^"]+' "$AWF" 2>/dev/null || true)
  if [ -n "$current_dir" ] && [[ "$current_dir" == /tmp/* ]]; then
    log "Fixing active_workspace.toml: ${current_dir} -> ${TOPCLAW_CONFIG_DIR}"
    echo "config_dir = \"${TOPCLAW_CONFIG_DIR}\"" > "$AWF"
  fi
fi

# Ensure quota tools are auto-approved (non-destructive: only adds if missing)
CFG="${TOPCLAW_CONFIG_DIR}/config.toml"
...
if [ ! -x "$TOPCLAW_BIN" ]; then
  logc "${RED}Binary not found: ${TOPCLAW_BIN}${NC}\n"
  exit 1
fi
...
log ""
log "OAuth profiles:"
if [ -f "${TOPCLAW_CONFIG_DIR}/auth-profiles.json" ]; then
  jq -r '.profiles | keys[]' "${TOPCLAW_CONFIG_DIR}/auth-profiles.json" 2>/dev/null | tee -a "$LOG_FILE" || log "(parse error)"
else
  log "(none found)"
fi
...
if timeout 10 ${TOPCLAW_BIN} agent -m 'respond OK' -p openai-codex 2>&1 | grep -qi "OK"; then
...
switch_active_profile() {
  ...
  local ap_file="${TOPCLAW_CONFIG_DIR}/auth-profiles.json"
...
AP_FILE="${TOPCLAW_CONFIG_DIR}/auth-profiles.json"
ORIG_ACTIVE_GEMINI=""
ORIG_ACTIVE_CODEX=""
if [ -f "$AP_FILE" ]; then
