...
//!
//! These tests require real auth-profiles.json at ~/.topclaw/auth-profiles.json
...
use std::sync::Arc;
use topclaw::config::Config;
use topclaw::tools::quota_tools::{
    CheckProviderQuotaTool, EstimateQuotaCostTool, SwitchProviderTool,
};
use topclaw::tools::Tool;

fn topclaw_dir() -> PathBuf {
    PathBuf::from(std::env::var("HOME").expect("HOME not set")).join(".topclaw")
}

fn has_auth_profiles() -> bool {
    topclaw_dir().join("auth-profiles.json").exists()
}

fn live_config() -> Config {
    Config {
        workspace_dir: topclaw_dir(),
...
fn error_parser_real_world_payloads() {
    use topclaw::providers::error_parser;
