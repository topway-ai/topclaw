//! Flash TopClaw Arduino firmware via arduino-cli.
//!
...
/// TopClaw Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/topclaw-arduino/topclaw-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "topclaw-arduino";
...
/// Flash TopClaw firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("topclaw_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling TopClaw Arduino firmware...");
    ...
    println!("TopClaw firmware flashed successfully.");
