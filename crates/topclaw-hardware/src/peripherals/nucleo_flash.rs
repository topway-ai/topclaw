//! Flash TopClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash TopClaw Nucleo firmware. Builds from firmware/topclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (topclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("topclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from topclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building TopClaw Nucleo firmware...");
    ...
    println!("TopClaw Nucleo firmware flashed successfully.");
